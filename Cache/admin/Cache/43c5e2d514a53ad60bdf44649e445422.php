...
<div id="nav" class="mainnav_title">
	<ul>
		<a class="on" href="<?php echo U('show_list');?>">权利清单列表</a> |
		<a href="<?php echo U('add');?>">添加权利清单</a>
	</ul>
</div>
<form action="" method="get">
	<table class="search_table" width="100%">
		<tr>
			<td class="search">清单名称: <input id="projectname" type="text" class="input-text" name="projectname" value="<?php echo ($projectname); ?>">
...
<form name="myform" id="myform" action="" method="post">
	<div class="table-list">
		<table cellspacing="0" cellpadding="0" width="100%" id="treetable">
			...
			<tbody>
				<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="ulbc" id="tr_<?php echo ($vo["id"]); ?>" data-tt-id="<?php echo ($vo["id"]); ?>"  <?php if(($vo["parentid"]) != ""): ?>data-tt-parent-id="<?php echo ($vo["parentid"]); ?>"<?php endif; ?> >
					<td id="td_sel">
					<input type="checkbox" class="sel" id="sel_<?php echo ($vo["id"]); ?>" value="<?php echo ($vo["id"]); ?>" name="items_<?php echo ($vo["id"]); ?>">
						</td>
					<td style="text-align: left;">
					<?php if(($vo["haschild"]) != "0"): ?><span class="folder ui-draggable"><?php echo ($vo["projectname"]); ?></span><?php endif; ?>
					<?php if(($vo["haschild"]) == "0"): ?><span class="file ui-draggable"><?php echo ($vo["projectname"]); ?></span><?php endif; ?>
					</td>
					<td style="text-align: left;"><?php echo ($vo["departname"]); ?></td>
					<td style="text-align: left;"><?php echo ($vo["classname"]); ?></td>
					<td style="text-align: left;"><?php echo ($vo["standard"]); ?></td>
					<td style="text-align: left;"><?php echo ($vo["taxevidence"]); ?></td>
					<td style="text-align: left;"><?php echo ($vo["timelimit"]); ?></td>
					<td style="text-align: left;"><?php echo ($vo["holddepart"]); ?></td>
					<td style="text-align: left;"><?php echo ($vo["tel"]); ?></td>
					<td><a
						href="__URL__/edit/id/<?php echo ($vo["id"]); ?>">编辑</a>&nbsp;|&nbsp; <a
						href="__URL__/del/id/<?php echo ($vo["id"]); ?>"
	...
	<div class="btn">
		<input type="submit" class="button" name="dosubmit" value="删除" onclick="myform.action='<?php echo U('deleteall');?>';"> 
	</div>
</form>
