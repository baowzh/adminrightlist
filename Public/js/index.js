...
function showdepartlist() {
	var display=$('#departlist').css('display');
	if(display=='none'){
		$('#seldept').css('width',50);
		$('#searchdept').append($('#departlist'));
		$('#departlist').show();
	}else{
		$('#seldept').css('width',230);
		$('#departlist').hide();
		$('#searchdept').remove('#departlist');
	}
	
};
var setCurrentDepart=function(deptid,deptname){
	$('#seldept').text(deptname);
	$('#departid').val(deptid);
	showdepartlist();
}
